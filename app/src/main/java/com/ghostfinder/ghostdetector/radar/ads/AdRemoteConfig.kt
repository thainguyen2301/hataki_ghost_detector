...
import com.ghostfinder.ghostdetector.radar.ads.model.AdConfiguration
import com.ghostfinder.ghostdetector.radar.model.HiddenAdsVersionConfig
import com.ghostfinder.ghostdetector.radar.model.LanguageScreenConfig
...
object AdRemoteConfig {
    ...
    const val KEY_AD_CONFIGURATION = "ad_configuration"

    const val KEY_DISABLE_INTERSTITIAL_SUCCESS_AD = "is_disable_interstitial_success_ad"
    const val KEY_DISABLE_INTERSTITIAL_BACK_AD = "is_disable_interstitial_back_ad"

    const val KEY_DISABLE_SPLASH_APP_OPEN_AD = "is_disable_splash_open_ad"

    private const val TAG = "AdRemoteConfig"
    var jsonObject: JSONObject? = null
    private var hiddenAdsVersionConfig: HiddenAdsVersionConfig? = null
    private val remoteConfig: FirebaseRemoteConfig = FirebaseRemoteConfig.getInstance()
    private var languageScreenConfig: LanguageScreenConfig? = null
    var adConfiguration: AdConfiguration? = null
        private set
    ...
    suspend fun initializeRemoteConfig(eventTracker: EventTracker, context: Context): Boolean {
        ...
        val defaults = mapOf(
            ...
            KEY_DISABLE_NATIVE_FULL_AD to true,
            ...
            KEY_DISABLE_SPLASH_APP_OPEN_AD to false,
            KEY_DISABLE_NATIVE_FULL_SCREEN_AFTER_HOME to true,
            KEY_DISABLE_NATIVE_FULL_SCREEN_AFTER_INTRO to true,
            KEY_DISABLE_NATIVE_FULL_SCREEN_AFTER_SPLASH to true,
            KEY_DISABLE_INTERSTITIAL_SUCCESS_AD to true,
            KEY_DISABLE_INTERSTITIAL_BACK_AD to true
        )
        ...
        return try {
            ...
            eventTracker.log(context, "Event === Remote Raw $jsonString ===")
...
//            printAllConfigValues(context)
            ...
            fetchAdConfiguration(context)

            result
        } catch (e: Exception) {
            Log.e(TAG, "Event get remote error due to ${e.toString()}")
            false
        }
    ...
    private fun fetchHiddenAdsVersionConfig(context: Context) {
        try {
            ...
            eventTracker.log(context, "Event === Hidden Ads Version Raw: $hiddenAdsVersionJson ===")
            ...
            if (hiddenAdsVersionJson.isNotEmpty()) {
                ...
                eventTracker.log(
                    context,
                    "Event === Hidden Ads Versions: ${
                        hiddenAdsVersionConfig?.versions?.joinToString(", ")
                    } ==="
                )
            } else {
                eventTracker.log(context, "Event === Hidden Ads Version is empty ===")
            }
        } catch (e: Exception) {
            Log.e(TAG, "Error fetching hidden_ads_version: ${e.message}", e)
            eventTracker.log(
                context,
                "Event === Error fetching hidden_ads_version: ${e.message} ==="
            )
        }
    }
    ...
    fun isAdEnabled(key: String): Boolean {
        try {
            val isDisabled = jsonObject?.getBoolean(key) ?: true
            return !isDisabled
        } catch (e: Exception) {
            return false
        }
    ...
    fun shouldShowSplashAppOpenAd(): Boolean = isAdEnabled(KEY_DISABLE_SPLASH_APP_OPEN_AD)
    ...
    fun shouldShowInterstitialSuccessAd(): Boolean = isAdEnabled(
        KEY_DISABLE_INTERSTITIAL_SUCCESS_AD
    )

    fun shouldShowNativeSuccessAd(): Boolean = isAdEnabled(KEY_DISABLE_NATIVE_SUCCESS_AD)
    fun shouldShowInterBackAd(): Boolean = isAdEnabled(
        KEY_DISABLE_INTERSTITIAL_BACK_AD
    )
    ...
    fun printAllConfigValues(context: Context) {
        ...
        keys.forEach { key ->
            ...
            value?.let { eventTracker.log(context, "Event $key: $value (Ad Enabled: ${!it})") }
        }
    ...
    private fun fetchLanguageScreenConfig(context: Context) {
        try {
            ...
            eventTracker.log(
                context,
                "Event === Language Screen Config Raw: $languageScreenConfigJson ==="
            )

            if (languageScreenConfigJson.isNotEmpty()) {
                val gson = Gson()
                languageScreenConfig =
                    gson.fromJson(languageScreenConfigJson, LanguageScreenConfig::class.java)

                eventTracker.log(
                    context, "Event === Language Screen Config: " +
                ...
                eventTracker.log(
                    context,
                    "Event === Language Screen Config is empty, using defaults ==="
                )
            }
        } catch (e: Exception) {
            ...
            eventTracker.log(
                context,
                "Event === Error fetching language_screen_config: ${e.message} ==="
            )
        }
    }
    ...
    fun getLanguageScreenConfig(): LanguageScreenConfig {
        ...
        languageScreenConfig?.let {
            if (it.ignoreVersions?.contains(version) == true) {
                return LanguageScreenConfig.default
            }
            return it
        }
    ...
    private fun fetchAdConfiguration(context: Context) {
        try {
            val adConfigurationJson = remoteConfig.getString(KEY_AD_CONFIGURATION)
            eventTracker.log(
                context,
                "Event === Ad Configuration Raw: $adConfigurationJson ==="
            )

            if (adConfigurationJson.isNotEmpty()) {
                val gson = Gson()
                adConfiguration =
                    gson.fromJson(adConfigurationJson, AdConfiguration::class.java)

                eventTracker.log(
                    context,
                    "Event === Ad Configuration: adInterRatio=${adConfiguration?.splashInterAdRatio} ==="
                )
            } else {
                eventTracker.log(context, "Event === Ad Configuration is empty ===")
            }
        } catch (e: Exception) {
            Log.e(TAG, "Error fetching ad_configuration: ${e.message}", e)
            eventTracker.log(
                context,
                "Event === Error fetching ad_configuration: ${e.message} ==="
            )
        }
    }

    fun getRandomAdType(context: Context): SplashFullAdType {
        val ratioValue = adConfiguration?.splashInterAdRatio ?: 100
        val randomValue = (1..100).random()
        eventTracker.log(
            context, "random ratio value: $randomValue"
        )
        return if (randomValue <= ratioValue) {
            SplashFullAdType.INTERSTITIAL
        } else {
            SplashFullAdType.OPEN_RESUME
        }
    }
}