package com.ghostfinder.ghostdetector.radar.ads.logger
...
class FirebaseEventTracker : EventTracker {
    ...
    companion object {
        private const val TAG = "FirebaseEventTracker"
    }
    ...
    override fun log(context: Context?, name: String, bundle: Bundle) {
        ...
        if (ctx == null) {
            AppAdvertiseManager.fileLogger.log(null,"Both context and androidContext are null, cannot log Firebase event: $name")
            return
        }
        ...
        try {
            ...
            AppAdvertiseManager.fileLogger.log(ctx, name, enrichedBundle)
        } catch (e: Exception) {
            AppAdvertiseManager.fileLogger.log(ctx, name, "Error logging Firebase event: ${e.message}")
        }
    }
